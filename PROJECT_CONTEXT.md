...
7. ~~**Conteúdo: arquétipos secundários menos cobertos.** `digital_antes_da_base` ...~~ **RESOLVIDO em 13/06 (scoringRules 1.3).** O arquétipo tinha peso espalhado e fora da tese, e na prática nunca vencia. Reformulado pra modelar "digital forte E base fraca": "divulga com frequência" passou a dar digital +3, "nunca calculei lucro/custo" dão digital +1 cada, removidos os sinais off-tese (empréstimo, "quero aparecer mais"), e criado piso próprio `minScorePerArchetype.digital_antes_da_base = 4` (só posta-muito não basta; precisa ter buraco de base junto). Nome trocado pra "Já apareço na internet, mas preciso de ajuda com o resto" (id mantido). Tests 12 e 13 cobrem o caso.
8. **Conteúdo: companheiros pra semanas 2-4 dos arquétipos não-Marlene-não-Amina.** Hoje Marlene tem o arco completo. `talento_sem_postura_comercial` ganhou companion de semana 3 (`companion_dizer_o_preco_sem_dobrar_carmen`) na auditoria de 13/06. Outros ainda têm só semana 1.
