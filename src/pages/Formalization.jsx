...
import questionsData from '../data/formalizationQuestions.json';
import guidesData from '../data/formalizationGuides.json';
import { scoreFormalization } from '../utils/formalizationScoring';
...
export default function Formalization() {
  ...
  const selectAnswer = (optId) => {
    ...
    setTimeout(() => {
      if (index === questions.length - 1) {
        const res = scoreFormalization(next, questions);
        setResult(res);
  ...
  const recommendedGuide = result
    ? guidesData.find((g) => g.id === result.recommendedId)
    : null;

  const openGuide = openGuideId
    ? guidesData.find((g) => g.id === openGuideId)
    : null;
    ...
    return (
      <div className="space-y-5">
        ...
        <h1 className="font-sans font-bold text-2xl text-ink leading-tight">
          PF, MEI ou CNPJ — qual cabe agora?
        </h1>
        <WavyUnderline className="w-40 h-3" />

        <p className="text-secondary text-base leading-relaxed">
          Cinco perguntas rápidas pra entender em que situação você está e
          qual o próximo passo de formalização faz sentido.
        </p>
          ...
          <div className="flex gap-3 items-start">
            ...
            <div>
              <p className="text-ink text-sm leading-relaxed mb-1">
                <strong>Importante:</strong> a Trilha não substitui contador.
              </p>
              <p className="text-secondary text-sm leading-relaxed">
                Damos um norte pra você não perder tempo, mas decisões
                tributárias específicas precisam de um contador ou de
  ...
  return (
    <div className="space-y-5">
      ...
      {result && (
        <p className="text-secondary text-sm leading-relaxed">
          Com base nas suas respostas, esse caminho parece ser o que mais
          combina agora. Você pode ver os outros dois guias abaixo a qualquer
          momento.
        </p>
      )}
        ...
        {guidesData.map((g) => {
          ...
          return (
            <button
              ...
              className={`px-3 pb-2 -mb-px text-sm font-semibold border-b-2 transition-colors ${
                isOpen
            ...
            >
              {g.id === 'pf' && 'PF'}
              {g.id === 'mei' && 'MEI'}
              {g.id === 'cnpj_outros' && 'CNPJ'}
              {isRec && (
                <Sparkle className="w-3 h-3 inline-block ml-1" />
              )}
            </button>
          );
        })}
      ...
      {openGuide && <GuideCard guide={openGuide} />}
        ...
        <div className="flex gap-3 items-start">
          ...
          <div>
            ...
            <p className="text-secondary text-xs leading-relaxed">
              Limites de faturamento e valores de imposto do MEI mudam ao
              longo do tempo. Antes de formalizar, confirme os valores atuais
              no Portal do Empreendedor (gov.br/empresas-e-negocios) ou com
              um contador.
            </p>
          </div>
        </div>
...
function GuideCard({ guide }) {
  return (
    <div className="space-y-4">
      ...
      <Card>
        <h3 className="font-bold text-ink mb-2">Como dar esse passo</h3>
        <p className="text-secondary text-sm leading-relaxed">
          {guide.howToFormalize}
        </p>
      </Card>
      ...
      <p className="text-xs text-secondary px-1">
        Última revisão deste guia: {guide.lastReviewed} · Próxima revisão prevista: {guide.nextReview}
      </p>
    </div>
  );
}
